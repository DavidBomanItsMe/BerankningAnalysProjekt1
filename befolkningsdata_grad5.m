...
ANew = [xNew.^5 xNew.^4 xNew.^3 xNew.^2 xNew ones(40, 1)];
...
s = 0;
for K = 1:40
    s = s + ((funUppsala(K,1) - y_Uppsala(K,1))^2);
end

s = sqrt(s);

felnorm_Uppsala_G5 = s;


felnorm_Jamtland_G5 = 0;
for K = 1:40
    felnorm_Jamtland_G5 = felnorm_Jamtland_G5 + ((funJamtland(K,1) - y_Jamtland(K,1))^2);
end

felnorm_Jamtland_G5 = sqrt(felnorm_Jamtland_G5);
