...
ANew = [xNew.^3 xNew.^2 xNew ones(40, 1)];
...
%syms kalle;
%uppsalaFelGrad3 = sqrt(symsum(((funUppsala(kalle,1) - y_Uppsala(kalle,1))^2), kalle, 1 ,40));

felnorm_Uppsala_G3 = 0;
for K = 1:40
    felnorm_Uppsala_G3 = felnorm_Uppsala_G3 + ((funUppsala(K,1) - y_Uppsala(K,1))^2);
end

felnorm_Uppsala_G3 = sqrt(felnorm_Uppsala_G3);


felnorm_Jamtland_G3 = 0;
for K = 1:40
    felnorm_Jamtland_G3 = felnorm_Jamtland_G3 + ((funJamtland(K,1) - y_Jamtland(K,1))^2);
end

felnorm_Jamtland_G3 = sqrt(felnorm_Jamtland_G3);
